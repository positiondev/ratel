...
spec = describe "Ratel" $ do
    describe "toError" $ do
        ...
        it "should abbreviate longer errors" $ do
          Exception.catch
                (do
                    _ <- error "something went wrong, so wrong, it was reeeeeeally wrong"
                    True `shouldBe` False)
                (\ exception -> do
                    let actual = Ratel.toError (exception :: Exception.SomeException)
                    let expected = Ratel.Error
                            ...
                            , Ratel.errorClass = Just "SomeException: something went wrong, so wrong"
                            , Ratel.errorMessage = Just "\
                                \something went wrong, so wrong, it was reeeeeeally wrong\n\
                                \CallStack (from HasCallStack):\n\
